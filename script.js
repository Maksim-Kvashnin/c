...
let cw = document.querySelector('.countWorkShift'); // кол-во смен н/д на странице
let view = true; // для показа/скрытия букв смен дня
header.onclick = e => {
    view = !view;
    let show = document.querySelector('.active');
    view ? Array.from(document.querySelector('.workShift').children).forEach(el => el.id === localStorage.getItem('day') && el.classList.add('active')) : show.classList.remove('active');
    document.querySelector('.countWorkShift').style.display = view ? 'block' : 'none';
    date = new Date(fy(), gm() - 1, 1); // было 15)
    table(2);
}
...
function table(m = 0) {
  tableBody.innerHTML = '';
  let countShiftInNight = 0, countShiftInDay = 0; // кол-во смен ночь/день
  const createEl = (td, s) => {
...
if(s === 'д') td.title = `с 8:00 по 20:00`;
else {
  let lastDayOfMonth = new Date((new Date(fy(),gm(),1)).setDate(0)).getDate();
  td.title = `c ${date.getDate() === 1 ? lastDayOfMonth : date.getDate() - 1}.${date.getDate() === 1 ? gm() : gm() + 1}.${String(fy()).slice(-2)} 20:00
по ${date.getDate()}.${gm() + 1}.${String(fy()).slice(-2)} 8:00`;
}
    td.append(symbolDay);
  } // String(gm()).padStart(2,0) - для первого нуля по 9-й месяц
    ...
    for(let j = 0; j <= 6; j++) { // цикл заполнение недели днями
      ...
      if(view) {
        ...
        if (shiftInDay === 0 || shiftInDay === 1) {
          ...
          if (date.toLocaleString('default', { month: 'long' }) === monthNameToo.textContent) countShiftInDay++;
        } else if (shiftInDay === 4 || shiftInDay === 5) {
          td.classList.add('night');   // подчёркивание цифры дня, ночной смены
          if (date.toLocaleString('default', { month: 'long' }) === monthNameToo.textContent) countShiftInNight++;
        ...
        cw.textContent = `${countShiftInNight} / ${countShiftInDay}`;
      }
...
if (showModal) {
  window.addEventListener('DOMContentLoaded', () => setTimeout(() => {
      document.querySelectorAll('.modal').forEach(el => el.classList.add('transparent'));
      showModal = false;
      localStorage.setItem('showModal', showModal);
    }, 3 * 1000)
  )
} else {
  document.querySelectorAll('.modal').forEach(el => el.style.opacity = 0);
}